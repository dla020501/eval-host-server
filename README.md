...
에피소드가 실패**합니다. 태스크가 잠근 자유도(아래 과제 표의 Task C 오른손 그리퍼·lift·base)는
보내도 **평가 서버가 0(=유지)으로 마스킹**합니다(누락 채움과는 다른 개념).

### 과제 — reset 의 `task_id` 와 과제별 계약

| `task_id` | 과제 | 에피소드 | 상한 | 에피소드 만점 | 잠기는 자유도 |
|---|---|---|---|---|---|
| `ConvStore-Task-A` | 바구니 운반(주행) | 3 | 12,000틱 (sim 10분) | 21 | 없음 — 22차원 전부 자유 |
| `ConvStore-Task-B` | 상자→진열대 진열 | 3 | 12,000틱 (sim 10분) | 30 | 없음 — 22차원 전부 자유 |
| `ConvStore-Task-C` | 계산대 QR 스캔 | 3 | 12,000틱 (sim 10분) | 51 | **오른손 그리퍼(14)·lift(18)·base(19–21)** — 스캐너를 쥔 오른손은 고정, 팔은 자유 |

- 세 과제 모두 `action_dim=22`, `control_hz=20`. 점수는 에피소드 **합산**(A 63 / B 90 / C 153).
- 시도 N의 씬은 고정(에피소드 순번 = 씬 번호). `server_info.info`의 `"Task Z-N"`으로 어느 과제·시도인지 알 수 있습니다.
