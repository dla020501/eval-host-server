...
제출 페이지에서 발급받은 **제출 토큰**을 서버에 걸어 두면, 평가 서버가 보내는
...
`ws://`는 평문입니다. 도메인이 있으면 정식 인증서(Let's Encrypt 등)로 `wss://도메인:포트`를
...
1. 정책 서버를 **공인 IP(또는 도메인)의 열린 포트**로 띄웁니다. 방화벽/공유기에서 해당
   포트 인바운드를 허용해야 평가 서버가 접속할 수 있습니다.
2. 제출 페이지에 접속해 팀 로그인 후, 서버 주소를 `ws://<공인IP>:<포트>`
   (TLS면 `wss://...`, self-signed면 인증서 지문도 함께) 형식으로 등록합니다.

   ![제출 폼 — 서버 주소 등록](./figure/submit-form.png)

3. 발급된 제출 토큰을 복사해 4번 섹션처럼 `--token`으로 서버에 걸어 둡니다.

   ![제출 토큰 발급](./figure/token-issue.png)

4. 제출하면 평가가 큐에 들어가고, 진행 상태와 점수는 제출 페이지에서 확인합니다.
