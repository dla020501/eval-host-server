"""데모 정책 서버. 통신 규약만 맞춰 제로 액션을 돌려주고, 평가 서버가 보내온 관측을
...
스택을 붙이기 전 배선 검증에는 충분하다. depth는 시뮬레이터의 이상화 값이다
(server.decode_depth 주석 참조). 손목에는 depth가 없다.
...
from server import BasePolicy, cli, decode_depth, serve_policy
...
def _scan_preview(scan: np.ndarray, size: int = 480, view_m: float = 5.0) -> np.ndarray:
    ...
    빈 i의 각도는 -pi + i*(2*pi/960) (base_link atan2(y,x)), 값은 거리(m), 20은 무반사.
    view_m 반경까지만 그린다."""
...
def _depth_preview(depth_m: np.ndarray) -> np.ndarray:
    """이상화 depth(미터, 0=무효)를 사람이 보기 위한 그레이 RGB로. 가까울수록 밝게."""
    valid = depth_m > 0
    if not valid.any():
